...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int i = 0; i < word.length(); i++) {
			...
			for (int j = 0; j < vowels.length(); j++) {
				if (vowels.charAt(j) == word.charAt(i)) { isVowel=true; }
			}
			if (isVowel && !lastWasVowel) {	++syll;	}
			if (i==word.length()-1 && word.charAt(i)=='e' && syll > 1 && !lastWasVowel) { --syll; }
	...
	public double getFleschScore()
	{
		...
		a = 206.835 - (1.015*((double)(wordsFound)/(double)(sentFound))) - (84.6*(double)(syllFound)/(double)(wordsFound));
